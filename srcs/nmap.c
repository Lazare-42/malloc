...
struct s_page       *fptr_stc_init_blocks_in_page(struct s_page *new_page, struct s_block  *pssd_new_block, uint64_t block_size)
{
...
struct s_page *fv__init_new_page(struct s_page *ptr_pssd_stc_new_page, uint64_t u64_pssd_required_size, uint64_t u64_pssd_minimum_initialization_size, uint64_t u64_pssd_number_of_structures_for_asked_size)
{
    ...
    ptr_pssd_stc_new_page->u64_number_of_used_blocks_in_page_ = u64_pssd_number_of_structures_for_asked_size;
...
struct s_page *fu8__create_and_init_new_page_category(struct s_manipulation *ptr_pssd_stc_manipulation_structure, uint64_t u64_pssd_required_size, uint64_t u64_pssd_number_of_elements)
{
    ...
    u64_lcl_minimum_initialization_size =
    ...
    u64_lcl_minimum_initialization_size, u64_pssd_required_size);
    ...
    if (NULL == (ptr_lcl_stc_new_page = fptr_void__nmap(u64_lcl_minimum_initialization_size)))
        return NULL;
    ptr_lcl_stc_new_page = fv__init_new_page(ptr_lcl_stc_new_page, u64_pssd_required_size, u64_lcl_minimum_initialization_size, u64_lcl_number_of_structures_for_asked_size);
    ptr_stc_lcl_new_block = (struct s_block*)&((uint8_t*)ptr_lcl_stc_new_page)[fu64__align16(sizeof(struct s_page))];
    return (fptr_stc_init_blocks_in_page(ptr_lcl_stc_new_page, ptr_stc_lcl_new_block, ptr_lcl_stc_new_page->u64_block_size_));
}
