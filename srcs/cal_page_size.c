...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/13 15:42:25 by lazrossi          #+#    #+#             */
/*   Updated: 2019/10/13 15:42:51 by lazrossi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
uint64_t	fu64__return_lower_page_size(
		uint64_t u64_pssd_next_page_size, uint64_t u64_pssd_required_size)
{
...
uint64_t	fu64__return_upper_page_size(uint64_t u64_previous_page_size,
...
uint64_t	fu64_get_upper_page_size_for_requested_size(struct s_manipulation
		*ptr_pssd_manipulation_structure, uint64_t u64_pssd_requested_size)
{
		...
		uint64_t u64_pssd_block_size, uint64_t u64_pssd_number_of_elements)
{
	...
	* (fu64__align16(sizeof(struct s_block))
	+ fu64__align16(u64_pssd_block_size));
	return (fu64_get_upper_page_size_for_requested_size(
			ptr_pssd_manipulation_structure, u64_lcl_requested_size));
}

uint64_t	fu64__get_number_of_structure_in_asked_page(
		uint64_t u64_pssd_asked_page_size, uint64_t u64_pssd_asked_malloc_size)
{
	return ((u64_pssd_asked_page_size
	- fu64__align16(sizeof(struct s_page)))
	/ (fu64__align16(sizeof(struct s_block))
	+ fu64__align16(u64_pssd_asked_malloc_size)));
}
