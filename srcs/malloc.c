...
/*	 Updated: 2019/10/13 11:42:51 by lazrossi		  ###	########.fr		  */
/*																			  */
/* ************************************************************************** */
...
struct s_page *fu8__add_upper_page_category_or_extend_current_page_category(struct s_manipulation *ptr_pssd_stc_manipulation_structure, struct s_page *ptr_pssd_stc_last_page, uint64_t u64_pssd_required_size)
{
	...
	if (NULL != (ptr_pssd_stc_last_page->ptr_next_page_in_category_ = fu8__create_and_init_new_page_category(ptr_pssd_stc_manipulation_structure, ptr_pssd_stc_last_page->u64_block_size_, ZERO)))
	{
...
struct s_page *fptr_page__return_corresponding_page_category(struct s_manipulation *ptr_pssd_stc_manipulation_structure, uint64_t u64_pssd_required_size)
{
	...
	if (ptr_stc_lcl_browse_page_category != NULL && fu8_bool__check_if_chosen_page_size_too_big(ptr_stc_lcl_browse_page_category->u64_block_size_, u64_pssd_required_size))
	{
...
struct s_page *fptr_page__return_first_free_block_from_page_category(struct s_manipulation *ptr_pssd_stc_manipulation_structure, struct s_page *ptr_pssd_stc_browse_page_category)
{
	...
	ptr_pssd_stc_browse_page_category = ptr_pssd_stc_browse_page_category->ptr_next_page_in_category_;
	}
	if (ptr_pssd_stc_browse_page_category == NULL) 
	{
		...
		if (NULL == (ptr_pssd_stc_browse_page_category = ptr_stc_lcl_previously_browsed_page->ptr_next_page_in_category_))
		{
			return (NULL);
		}
