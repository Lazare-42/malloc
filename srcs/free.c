...
uint8_t fu8__bool_check_if_malloced(struct s_manipulation *ptr_pssd_manipulation_structure, void *ptr_pssd_pointer_free)
{
    ...
    while (NULL != ptr_stc_lcl_page_category_to_browse)
    {
        ...
        while (NULL != ptr_stc_lcl_page_inside_category_to_browse)
        {
            ...
            {
                if ((void*)(((uint8_t*)ptr_stc_lcl_browse_alloced_blocks) + fu64__align16(sizeof(struct s_block))) == ptr_pssd_pointer_free)
                {
                    return ((uint8_t)1);
                }
...
void fvoid__free_half_of_used_pages_from_one_page(struct s_page *ptr_pssd_first_page_for_category)
{
